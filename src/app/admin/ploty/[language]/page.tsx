...
async function Validate(language: string) {
  ...
  if (browser_uid === process.env.ADMIN_UID) {
    ...
    const data = await GetAdminPloty(language);
    if (data) {
      return (
        <AdminPloty language={language} data={data} languages={languages} />
      );
    }
    console.log(language);

    return (
      <AdminPloty language={language} data={undefined} languages={languages} />
    );
  } else {
