"use client";
import { Gallery, Ostatne } from "@/app/lib/interface";

import Image from "next/image";

import { icon_text } from "@/app/lib/data";
import { BLUR_DATA_URL_GRAY } from "@/app/lib/functionsClient";
import { fetchGalleryType, fetchOstatne } from "@/app/lib/functionsServer";
...
const OstatnePage = () => {
  const { data, error, isLoading } = useQuery<Ostatne | null>({
    queryKey: ["ostatne", Cookies.get("language")],
    queryFn: () => fetchOstatne(Cookies.get("language")),
    staleTime: 1000 * 60 * 5,
  ...
  const {
    ...
    queryKey: ["galeria", "ostatne"],
    queryFn: () => fetchGalleryType("ostatne"),
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false,
    placeholderData: (previousData, previousQuery) => previousData,
  });
