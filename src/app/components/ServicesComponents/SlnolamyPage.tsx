...
import { fetchGalleryType, fetchSlnolamy } from "@/app/lib/functionsServer";
import ServiceSkeleton from "./ServiceSkeleton";

const SlnolamyPage = () => {
  const { data, error, isLoading } = useQuery<Slnolamy | null>({
    queryKey: ["slnolamy", Cookies.get("language")],
    queryFn: () => fetchSlnolamy(Cookies.get("language")),
    staleTime: 1000 * 60 * 5,
  ...
  const {
    ...
    queryKey: ["galeria", "slnolamy"],
    queryFn: () => fetchGalleryType("slnolamy"),
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false,
    placeholderData: (previousData, previousQuery) => previousData,
  });
    ...
    <main>
      ...
      {data && (
        <>
          ...
          <VisualizationElement data={data} service_type="slnolamy_pergola" />
        </>
      )}
    </main>
  );
