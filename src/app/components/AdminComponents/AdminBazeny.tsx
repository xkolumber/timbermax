...
import { empty_five_values, EmptyNacenovac } from "@/app/lib/functionsClient";
import { fetchBazeny, fetchBazenyAdmin } from "@/app/lib/functionsServer";
import { Bazeny } from "@/app/lib/interface";
...
const AdminBazeny = ({ language }: Props) => {
  const queryClient = useQueryClient();
  const [isLoading2, setIsLoading2] = useState(false);

  const { data, error, isLoading } = useQuery<Bazeny | null>({
    queryKey: ["admin_bazeny", language],
    queryFn: () => fetchBazenyAdmin(language),
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false,
    placeholderData: (previousData, previousQuery) => previousData,
  });

  const [actualizeData, setActualizeData] = useState<Bazeny>({
    nadpis: "",
    popis1: "",
    popis2: "",
  ...
  useEffect(() => {
    if (error) {
      setActualizeData((prevData) => ({
        ...
        postup_popis: "",
        postup_nacenovac: "",
        nacenovac_sekcie: EmptyNacenovac,
        nadpis_vizualizacia: "",
        popis_viz_1: "",
        farba: "",
        btn_ceny: "",
  ...
  const handleChangeItemArray = (
    ...
    setActualizeData((prevData) => {
      const updatedArray = [...(prevData[title as keyof Bazeny] as string[])];
  ...
  const handleChangeItemTwoArray = (
    ...
    setActualizeData((prevData) => {
      const updatedArray = [...(prevData[title as keyof Bazeny] as any[])];
      updatedArray[index] = {
        ...updatedArray[index],
  ...
  const handleSaveProduct = async (e: any) => {
    ...
    const response = await AdminActualizeBazenyPage(actualizeData, language);
    setIsLoading2(false);
    if (response === 200) {
