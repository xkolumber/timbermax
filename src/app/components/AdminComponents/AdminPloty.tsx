...
import { fetchPloty, fetchSlnolamy } from "@/app/lib/functionsServer";
...
const AdminPloty = ({ language }: Props) => {
  const queryClient = useQueryClient();

  const { data, error, isLoading } = useQuery<Slnolamy | null>({
    queryKey: ["admin_ploty", language],
    queryFn: () => fetchPloty(language),
    staleTime: 1000 * 60 * 5,
