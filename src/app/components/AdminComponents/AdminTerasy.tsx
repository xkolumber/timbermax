"use client";
import { AdminActualizeTerasyPage } from "@/app/lib/actions";
import {
  empty_five_values,
  empty_three_values,
} from "@/app/lib/functionsClient";
import { fetchTerasy, fetchTerasyAdmin } from "@/app/lib/functionsServer";
import { Ploty, Terasy } from "@/app/lib/interface";
...
const AdminTerasy = ({ language }: Props) => {
  const queryClient = useQueryClient();
  const { data, error, isLoading } = useQuery<Terasy | null>({
    queryKey: ["admin_terasy", language],
    queryFn: () => fetchTerasyAdmin(language),
    staleTime: 1000 * 60 * 5,
  ...
  const [actualizeData, setActualizeData] = useState<Terasy>({
    nadpis: "",
    popis1: "",
    popis2: "",
  ...
  const handleSaveProduct = async (e: any) => {
    ...
    const response = await AdminActualizeTerasyPage(language, actualizeData);
    setIsLoading2(false);
    if (response === 200) {
            ...
            <div className="flex flex-col">
              {actualizeData.vlastnosti.map((size, index) => (
                <input
                  ...
                  value={size}
                  onChange={(e) =>
                    handleChangeItemArray("vlastnosti", index, e)
                  }
