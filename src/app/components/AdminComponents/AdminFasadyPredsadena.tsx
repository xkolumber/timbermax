"use client";
import { AdminActualizeFasadyPresadenaPage } from "@/app/lib/actions";
import { empty_five_values } from "@/app/lib/functionsClient";
import { fetchFasady, fetchFasadyAdmin } from "@/app/lib/functionsServer";
...
const AdminFasadyPredsadena = ({ language }: Props) => {
  const queryClient = useQueryClient();
  const { data, error, isLoading } = useQuery<Fasady | null>({
    queryKey: ["admin_fasady_predsadena", language],
    queryFn: () => fetchFasadyAdmin("fasady-predsadena", language),
    staleTime: 1000 * 60 * 5,
  ...
  const handleSaveProduct = async (e: any) => {
    ...
    const response = await AdminActualizeFasadyPresadenaPage(
      actualizeData,
      language
    );
    setIsLoading2(false);
    ...
    <div className="">
      ...
      {error && (
        <form className="products_admin" onSubmit={handleSaveProduct}>
          <h4>Sekcia Fasady - predsadena</h4>
...
export default AdminFasadyPredsadena;
