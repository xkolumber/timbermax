...
import { fetchOstatne, fetchOstatneAdmin } from "@/app/lib/functionsServer";
import { Ostatne, Ploty } from "@/app/lib/interface";
...
const AdminOstatne = ({ language }: Props) => {
  const queryClient = useQueryClient();

  const { data, error, isLoading } = useQuery<Ostatne | null>({
    queryKey: ["admin_ostatne", language],
    queryFn: () => fetchOstatneAdmin(language),
    staleTime: 1000 * 60 * 5,
  ...
  const [actualizeData, setActualizeData] = useState<Ostatne>({
    nadpis: "",
    popis1: "",
    popis2: "",
  ...
  const handleSaveProduct = async (e: any) => {
    ...
    const response = await AdminActualizeOstatnePage(actualizeData, language);
    setIsLoading2(false);
    if (response === 200) {
